...
namespace OS2DSRules {
  ...
  namespace CPRDetector {
    ...
    constexpr CPRDetector& CPRDetector::operator=(const CPRDetector& other) noexcept {
      if (this != &other) {
	check_mod11_ = other.check_mod11_;
	examine_context_ = other.examine_context_;
      }
    ...
    constexpr CPRDetector::CPRDetector(CPRDetector&& other) noexcept
      : check_mod11_(std::move(other.check_mod11_)), examine_context_(std::move(other.examine_context_)) {}
    ...
    char CPRDetector::update(char c, CPRDetectorState new_state, CPRDetectorState& old_state, Predicate is_acceptable) noexcept {
      if (is_acceptable(c)) {
    ...
    bool CPRDetector::check_day_month(const std::string& cpr, CPRDetectorState& state) noexcept {
    ...
    void CPRDetector::check_leap_year(const std::string& cpr, CPRDetectorState& state) noexcept {
	...
	case CPRDetectorState::First:
	  ...
	  previous = cpr[1] = update(*it, CPRDetectorState::Second, state, is_acceptable);
	...
	case CPRDetectorState::Second:
	  ...
	  is_acceptable = make_predicate('0', '1');
	  previous = cpr[2] = update(*it, CPRDetectorState::Third, state, is_acceptable);
	...
	case CPRDetectorState::Third:
	  ...
	  previous = cpr[3] = update(*it, CPRDetectorState::Fourth, state, is_acceptable);
	...
	case CPRDetectorState::Fourth:
	  ...
	  previous = cpr[4] = update(*it, CPRDetectorState::Fifth, state, is_acceptable);
	...
	case CPRDetectorState::Fifth:
	  ...
	  previous = cpr[5] = update(*it, CPRDetectorState::Sixth, state, is_acceptable);
	...
	case CPRDetectorState::Sixth:
	  ...
	  is_acceptable = is_digit;
	  previous = cpr[6] = update(*it, CPRDetectorState::Seventh, state, is_acceptable);
	
	  break;
	case CPRDetectorState::Seventh:
	  is_acceptable = is_digit;
	  previous = cpr[7] = update(*it, CPRDetectorState::Eighth, state, is_acceptable);

	  break;
	case CPRDetectorState::Eighth:
	  is_acceptable = is_digit;
	  previous = cpr[8] = update(*it, CPRDetectorState::Match, state, is_acceptable);
