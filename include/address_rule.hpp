...
private:
  ...
  filter_matches(const MatchResults &matches, const std::string &) const noexcept;
  [[nodiscard]] std::optional<MatchResult> append_number(const MatchResult &, const std::string &) const noexcept;
};
}; // namespace AddressRule
